...
<head>
  <meta charset="UTF-8" />
  <title>405 – Method Not Allowed · ChainScholar</title>
...
<body class="min-h-screen bg-white flex items-center justify-center p-6">
  <main class="w-full max-w-2xl text-center">
    ...
    <h1 class="text-2xl md:text-3xl font-bold text-slate-900">405 — Method Not Allowed</h1>
    <p class="mt-3 text-slate-600 text-sm md:text-base">
      That URL doesn’t support this action. (Example: logging out must be a POST action from the app.)
    </p>

    @php $homeUrl = auth()->check() ? route('dashboard') : url('/'); @endphp
    <div class="mt-7 flex items-center justify-center gap-3">
      <a href="{{ url()->previous() }}" class="rounded-full border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">Go Back</a>
      ...
      @guest
        <a href="{{ route('login') }}" class="rounded-full border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">Return to Sign In</a>
      @endguest
    </div>
